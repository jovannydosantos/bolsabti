...
$pdf->SetTitle($oferta['CompanyJobProfile']['job_name']);
$pdf->SetSubject($oferta['CompanyJobProfile']['job_name']);
...
EOD;
  ...
  if($oferta['CompanyJobOffer']['show_details_responsible']=='s'): 
	...
	if($oferta['CompanyJobContractType']['mobility']=='s'):
	  	$dispviaj = '<table width="100%"><tr><td><strong>Disponibilidad para viajar:</strong> Sí</td></tr></table>';
	   	if($oferta['CompanyJobContractType']['mobility_option']=='1'):
		 	$dispviaj = '<table width="100%"><tr><td><strong>Disponibilidad para viajar:</strong> Dentro del país - '.$oferta['CompanyJobContractType']['mobility_city'].'</td></tr></table>';
	  	else:
	  		$dispviaj = '<table width="100%"><tr><td><strong>Disponibilidad para viajar:</strong> Fuera del país - '.$oferta['CompanyJobContractType']['mobility_city'].'</td></tr></table>';
	  	endif;
	else:
	...
	if($oferta['CompanyJobContractType']['change_residence']=='s'):
	  $dispchange = '<table width="100%"><tr><td><strong>Disponibilidad para cambiar de residencia:</strong> Sí</td></tr></table>';
		  if($oferta['CompanyJobContractType']['change_residence_option']=='1'):
		 $dispchange = '<table width="100%" <tr><td><strong>Disponibilidad para cambiar de residencia:</strong> Dentro del país - '.$oferta['CompanyJobContractType']['change_residence_state'].'</td></tr></table>';
		  else:
		  $dispchange = '<table width="100%"><tr><td><strong>Disponibilidad para cambiar de residencia:</strong> Fuera del país - '.$oferta['CompanyJobContractType']['change_residence_state'].'</td></tr></table>';
		  endif;
		else:
	...
	if($oferta['CompanyJobContractType']['other_benefits']<>''):
		$otrasPrestaciones .= '<table width="100%" ><tr><td><strong>Otras prestaciones: </strong>'.$oferta['CompanyJobContractType']['other_benefits'].'</td></tr></table>';
	endif;
	...
	if(!empty($oferta['CompanyJobLanguage'])):
		foreach($oferta['CompanyJobLanguage'] as $k => $idioma):
	        ...
	        $idio1 .= '<tr><td width="25%">'.$idioma['Lenguage']['lenguage'].'</td><td width="25%">Lectura- '.$NivelesIdioma[$idioma['reading_level']]. '</td><td width="25%">Escritura- '.$NivelesIdioma[$idioma['writing_level']]. '</td><td width="25%">'.$fcmdidio.'</td></tr>';  
			$numidio++;
		endforeach; 
	...
	if(!empty($oferta['CompanyJobComputingSkill'])):
		...
		$compu2 = '<br><table width="100%" ><tr><td><strong>Cómputo:</strong></td></tr></table>';
        if($numcompu > 0):
			$compu2 .='<table width="100%" >'.$compu1.'</table>';
		endif;
    endif;
  ...
  if(!empty( $oferta['CompanyJobContractType']['Workday']['workday'])):
	...
	$lugarJob = '';
	$lugarJob .= $oferta['CompanyJobContractType']['state'].' - '.$oferta['CompanyJobContractType']['subdivision'].' - '.$oferta['CompanyJobContractType']['location_reference']; 
	...
	<table width="100%" ><tr><td><strong>RESPONSABLE DE LA OFERTA</strong></td></tr></table>
	<table width="100%" ><tr><td>'.$conName.'</td><td width="50%">'.$conPuesto.'</td></tr></table>';

	if($conTelefono<>''):
		$html.= '<table width="100%" ><tr><td>'.$conTelefono.'</td><td>'.$conCel.'</td></tr></table>';
	endif;

	if($conEmail<>''):
		$html.= '<table width="100%" ><tr><td>'.$conEmail.'</td></tr></table>';
	endif;
	...
	if(!empty($oferta['CompanyCandidateProfile']) OR ($idio2<>'') OR ($compu2<>'') ):
		$html.='
		<table width="100%" >
		<tr>
...
?>
	<blockquote style="border-top-width: 0px;padding-top: 0px;padding-bottom: 0px;margin-top: 10px;margin-bottom: 5px;">
        <p style="color: #588BAD">Vista de oferta en pdf.</p>
    </blockquote>

	<div class="embed-responsive embed-responsive-16by9" style="margin-top: 15px;">
		<iframe src="<?php echo $this->webroot; ?>files/pdf/<?php echo $namePdf ?>" style="width:100%; height:650px;" frame allowfullscreen>Your browser does not support inline frames or is currently configured not to display inline frames.</iframe>
	</div>