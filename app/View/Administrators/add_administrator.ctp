	...
	<script>
		...
		<?php if(($this->request->data['AdministratorProfile']['start_date_expiration']<>'') and ($this->request->data['AdministratorProfile']['start_date_expiration'] <> '0000-00-00')): ?>
		...
		<?php if(isset($this->request->data['AdministratorProfile']) and (!empty($this->request->data['AdministratorProfile']))){ ?>
			...
			if($("#AdministratorProfileAcademicLevelId").val() != 0)
				...
				$.get('http://bolsa.trabajo.unam.mx/unam/app/webroot/php/derpEsc.php',{level: $('#AdministratorProfileAcademicLevelId').find(":selected").index() },function(JSON)
					...
					$.each(JSON, function(key, val){
						...
						// $("#AdministratorProfileInstitution option[value='"+dejarEscuelas[x]+"']").remove();
		...
		function validateDates(){
			...
			if(tiempoIlimitado == 0){
				...
				if((year1 == '') || (month1 == '') || (day1 == '') || (year2 == '') || (month2 == '') || (day2 == '')){
					...
					jAlert('La fecha inicio de la vigencia es incorrecta', 'Mensaje');
					document.getElementById('AdministratorProfileStartDateExpirationDay').focus();
					return false;
				} else if(vigenciaFecha2 == false){
					jAlert('La fecha fin de la vigencia es incorrecta', 'Mensaje');
					document.getElementById('AdministratorProfileEndDateExpirationDay').focus();
					return false;
				} else {
					return true;
				}
		...
		<div style="background-color: #835B06; margin-bottom: 15px; padding: 15px 0 0; " class="col-md-7 col-md-offset-2">
			...
			<div class="col-md-12"  style="margin-top: 30px;">
				 <?php echo $this->Form->input('Administrator.username', array(
								'before' => '<div class="col-md-11" ><img data-toggle="tooltip" id="" data-placement="right" title="Asigne un nombre al usuario que desea crear" alt="help.png" class="img-circle cambia" style="margin-top: 8px;" src="/unam/img/help.png">',
