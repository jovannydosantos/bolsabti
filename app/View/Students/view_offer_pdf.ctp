...
// print a block of text using Write()
	...
	//Atrapar disponibilidad para viajar y cambiar de residencia
		...
		$ban = 0;
          ...
          if($oferta['CompanyCandidateProfile']['licenciatura'] <> 0):
             $nivelAcademico .= '<br>Licenciatura'; 
			 $ban = 1; 
			 $ultimoNivel = 'Licenciatura'; 
          endif;
           
          if($oferta['CompanyCandidateProfile']['especialidad'] <> 0): 
            $nivelAcademico .= '<br>Especialidad';
			$ban = 1; 
			$ultimoNivel = 'Especialidad';
          endif;
           
          if($oferta['CompanyCandidateProfile']['maestria'] <> 0):
            $nivelAcademico .='<br>Maestria' ;
			$ban = 1; 
			$ultimoNivel = 'Maestria';
          endif;
           
          if($oferta['CompanyCandidateProfile']['doctorado'] <> 0):
            $nivelAcademico .= '<br>Doctorado' ;
			$ban = 1; 
			$ultimoNivel = 'Doctorado';
          endif;

         if($ban > 0):
            $nivelAcademico2 .= $situacionAcademica[$oferta['CompanyCandidateProfile']['academic_situation']];
            if($oferta['CompanyCandidateProfile']['academic_situation']==1):
             $nivelAcademico2 .=' ('.$oferta['CompanyCandidateProfile']['semester'].'° semestre)';
            endif;
        endif;
        
		$carrera = '';
		if(!empty($oferta['CompanyCandidateProfile']['CompanyJobRelatedCareer'])):
			foreach ($oferta['CompanyCandidateProfile']['CompanyJobRelatedCareer'] as $key => $carreras) {
				$carrera .= '<br>-'.$CarrerasAreas[$carreras['career_id']];
			}
		endif;
  ...
  if(!empty( $oferta['CompanyJobContractType']['Workday']['workday'])):
	...
	if($conSede<>''):
		$html.= '<table width="100%" border="0"><tr><td>'.$conSede.'</td></tr></table>';
	endif;
	...
	if(($nivelAcademico<>'') OR ($nivelAcademico2<>'') OR ($carrera<>'') OR ($idio2<>'') OR ($compu2<>'') ):
		$html.='
		<table width="100%" border="0">
		<tr>
		...
		if($nivelAcademico<>''):
			$html.='
				...
				<tr>
					<td><strong>'.$nivelAcademico.' '.$nivelAcademico2.'</strong></td>
				</tr>
				</table>';
		endif;
